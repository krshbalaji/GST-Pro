...
def test_demo_factory_remains_explicit():
    ...
    state["masters"]={"gstins":state["gstins"],"customers":state["customers"],"vendors":state["vendors"],"products":state["products"],"users":state["users"]}
    state["audit"]=[]; state["returns"]={}
...
def test_demo_invoice_contract_is_unchanged():
    rows={}; repo=DemoInvoiceRepository(rows)
...
def test_invoice_lifecycle_transition_contract():
    assert "PENDING_APPROVAL" in INVOICE_STATES["DRAFT"]
    assert "APPROVED" in INVOICE_STATES["PENDING_APPROVAL"]
    assert "REJECTED" in INVOICE_STATES["PENDING_APPROVAL"]
    assert "EINVOICE_GENERATED" in INVOICE_STATES["APPROVED"]
    validate_invoice_transition("DRAFT","PENDING_APPROVAL")
    try:
        validate_invoice_transition("DRAFT","EINVOICE_GENERATED")
        assert False,"illegal invoice transition should fail"
    except ValueError:
        pass
