...
def test_maker_checker_before_irn():
    ...
    r=client.post('/api/invoices',headers=owner,json=payload()); assert r.status_code==200; iid=r.json()['id']
...
def test_audit_chain():
    r=client.get('/api/audit-verify?company_id=demo-company'); assert r.status_code==200 and r.json()['valid'] is True
