...
def bootstrap_enabled() -> bool:
    return os.getenv("GSTPRO_BOOTSTRAP", "0").strip().lower() in {"1", "true", "yes"}


def bootstrap_production(store) -> None:
    """Create/update the local development tenant when explicitly enabled."""
    ...
    with store.connect() as conn:
        ...
        conn.execute(
            ...
            ON CONFLICT (id) DO UPDATE SET
              company_id=EXCLUDED.company_id,gstin=EXCLUDED.gstin,state_code=EXCLUDED.state_code,
              scheme=EXCLUDED.scheme,legal_name=EXCLUDED.legal_name,trade_name=EXCLUDED.trade_name,is_active=true
            """,
            (
                BOOTSTRAP_GSTIN_ID,BOOTSTRAP_COMPANY_ID,"33ABCDE1234F1Z5","33","REGULAR",
                "SARO DEVI ENTERPRISES","Saro Devi Enterprises",
            ),
        )

        users = [
            (BOOTSTRAP_OWNER_ID,"Admin","admin@gstpro.local","OWNER",hash_password("admin")),
            (BOOTSTRAP_CA_ID,"Demo CA","ca@gstpro.local","CA",hash_password("caadmin123")),
        ]
        for uid,name,email,role,password_hash in users:
            conn.execute(
                """
                INSERT INTO users (id,company_id,name,email,role,password_hash,is_active)
                VALUES (%s,%s,%s,%s,%s,%s,true)
                ON CONFLICT (email) DO UPDATE SET
                  company_id=EXCLUDED.company_id,name=EXCLUDED.name,role=EXCLUDED.role,
                  password_hash=EXCLUDED.password_hash,is_active=true
                """,
                (uid,BOOTSTRAP_COMPANY_ID,name,email,role,password_hash),
            )
        ...
        products = [
            (BOOTSTRAP_PRODUCT_1,"Cotton Shirt (Men)","620520","PCS",500,18,False),
            (BOOTSTRAP_PRODUCT_2,"Towel (Home Textile)","630260","PCS",300,12,False),
            (BOOTSTRAP_PRODUCT_3,"IT Consulting","998313","HRS",2500,18,True),
        ]
        for pid,description,hsn,unit,rate,gst_rate,is_service in products:
            conn.execute(
                """
                ...
                ON CONFLICT (id) DO NOTHING
                """,
                (pid,BOOTSTRAP_COMPANY_ID,description,hsn,unit,rate,gst_rate,is_service),
            )

        mappings = [
            ("company","demo-company",BOOTSTRAP_COMPANY_ID),
            ("gstin","demo-gstin",BOOTSTRAP_GSTIN_ID),
            ("user","U1",BOOTSTRAP_OWNER_ID),("user","U2",BOOTSTRAP_CA_ID),
            ("customer","C1",BOOTSTRAP_CUSTOMER_1),("customer","C2",BOOTSTRAP_CUSTOMER_2),
            ("product","P1",BOOTSTRAP_PRODUCT_1),("product","P2",BOOTSTRAP_PRODUCT_2),
        ...
        for entity_type,domain_id,database_id in mappings:
            conn.execute(
                ...
                VALUES (%s,%s,%s)
                ON CONFLICT (entity_type,domain_id) DO UPDATE SET database_id=EXCLUDED.database_id
                """,
