...
def _production_mapper():
    return REPOSITORIES['invoices'].mapper if REPOSITORIES is not None and not DEMO_MODE else None

def _canonical_company_id(company_id):
    if DEMO_MODE or REPOSITORIES is None:
        return company_id
...
def _domain_company_id(company_uuid):
    if DEMO_MODE or REPOSITORIES is None:
        return company_uuid
    # Prefer the compatibility mapping so API payloads retain stable domain IDs.
    try:
        with store.connect() as conn:
            row=conn.execute("SELECT domain_id FROM gstpro_id_map WHERE entity_type='company' AND database_id=%s LIMIT 1",(company_uuid,)).fetchone()
            return row['domain_id'] if row else str(company_uuid)
    except Exception:
        return str(company_uuid)

def _domain_gstin_id(gstin_uuid):
    if DEMO_MODE or REPOSITORIES is None:
        return gstin_uuid
    try:
        with store.connect() as conn:
            row=conn.execute("SELECT domain_id FROM gstpro_id_map WHERE entity_type='gstin' AND database_id=%s LIMIT 1",(gstin_uuid,)).fetchone()
            return row['domain_id'] if row else str(gstin_uuid)
    except Exception:
        return str(gstin_uuid)
...
def company_scope(user, company_id:str):
    if DEMO_MODE:
        ...
        return
    try:
        requested=_canonical_company_id(company_id)
        user_company=_canonical_company_id(user.get('company_id'))
    except (ValueError,TypeError) as exc:
        raise HTTPException(403, f'Cross-company access denied: {exc}')
...
def ensure_period_open(gstin_id:str, invoice_date:str):
    try:
        ...
        else: rows = RETURNS.values()
        compliance_service.ensure_period_open(rows, gstin_id, invoice_date)
    except ValueError as exc: raise HTTPException(409, str(exc))

def validate_invoice(req:InvoiceRequest):
    if req.invoice_type=='TAX_INVOICE' and req.scheme==Scheme.COMPOSITION:
        raise HTTPException(422,'Composition taxpayers must issue a Bill of Supply; GST must not be charged to the customer.')
    if not DEMO_MODE:
        try:
            gstin = GSTINS.get(req.gstin_id)
            if not gstin or _canonical_company_id(gstin.get('company_id')) != _canonical_company_id(req.company_id):
                raise HTTPException(422,'GSTIN does not belong to the selected company.')
        except (ValueError,TypeError):
            raise HTTPException(422,'GSTIN or company identifier is invalid.')
    ...
    if req.scheme==Scheme.COMPOSITION and req.invoice_type not in ('BILL_OF_SUPPLY','CREDIT_NOTE','DEBIT_NOTE'):
        raise HTTPException(422,'Composition supply requires Bill of Supply.')
    ...
    for l in req.lines:
        try:
            ...
            invoice_service.validate_hsn(l.hsn_sac, company.get('aato',0))
...
def invoice_row(req, calc, status='DRAFT'):
    iid=str(uuid.uuid4())
    row={'id':iid,'created_at':datetime.now(timezone.utc).isoformat(),'request':req.model_dump(),'calculation':{k:str(v) if isinstance(v,Decimal) else v for k,v in calc.items()},'status':status}
    try:
        if REPOSITORIES is not None and not DEMO_MODE:
            with transaction() as conn:
                row=REPOSITORIES['invoices'].create(row,conn=conn)
                audit_row={'id':str(uuid.uuid4()),'action':'CREATE','entity_type':'INVOICE','entity_id':row['id'],'new_value':row,'company_id':req.company_id,'user_id':None,'created_at':datetime.now(timezone.utc).isoformat(),'previous_hash':'GENESIS','hash':''}
                REPOSITORIES['audit'].append(audit_row,conn=conn)
        else:
            INVOICES[iid]=row; audit('CREATE','INVOICE',iid,row,company_id=req.company_id)
    except Exception as exc: raise HTTPException(409,f'Invoice could not be persisted: {exc}')
    persist_state(); return row
...
def companies(user=Depends(require_permission('read'))):
    if DEMO_MODE: return [x for x in COMPANIES.values() if x.get('id')==user.get('company_id')]
    return [REPOSITORIES['companies'].get(user.get('company_id'))]

@app.get('/api/gstins')
def gstins(company_id:str='demo-company',user=Depends(require_permission('read'))):
    company_scope(user,company_id)
    return [x for x in GSTINS.values() if _canonical_company_id(x['company_id'])==_canonical_company_id(company_id)]

@app.get('/api/customers')
def customers(company_id:str='demo-company',user=Depends(require_permission('read'))):
    company_scope(user,company_id)
    return [x for x in CUSTOMERS.values() if _canonical_company_id(x['company_id'])==_canonical_company_id(company_id)]
...
def vendors(company_id:str='demo-company',user=Depends(require_permission('read'))):
    company_scope(user,company_id)
    return [x for x in VENDORS.values() if _canonical_company_id(x['company_id'])==_canonical_company_id(company_id)]
...
def products(company_id:str='demo-company',user=Depends(require_permission('read'))):
    company_scope(user,company_id)
    return [x for x in PRODUCTS.values() if _canonical_company_id(x['company_id'])==_canonical_company_id(company_id) and x.get('active',True)]
...
def create(req:InvoiceRequest,user=Depends(require_permission('create'))):
    company_scope(user,req.company_id); ensure_period_open(req.gstin_id,req.invoice_date)
...
def get_invoice(invoice_id:str,user=Depends(require_permission('read'))):
    ...
    if REPOSITORIES is not None and not DEMO_MODE: inv=REPOSITORIES['invoices'].get(invoice_id)
    ...
    try:
        if REPOSITORIES is not None and not DEMO_MODE:
            with transaction() as conn:
                ...
                REPOSITORIES['audit'].append({'id':str(uuid.uuid4()),'action':'CANCEL','entity_type':'INVOICE','entity_id':invoice_id,'new_value':{'status':'CANCELLED'},'company_id':inv['request']['company_id'],'user_id':user['sub'],'created_at':datetime.now(timezone.utc).isoformat(),'previous_hash':'GENESIS','hash':''},conn=conn)
            inv=REPOSITORIES['invoices'].get(invoice_id)
        else:
            validate_invoice_transition(inv.get('status','DRAFT'),'CANCELLED'); inv['status']='CANCELLED'; audit('CANCEL','INVOICE',invoice_id,{'status':'CANCELLED'},company_id=inv['request']['company_id'],user_id=user['sub'])
...
def mock_einvoice(req:EinvoiceRequest,user=Depends(require_permission('edit'))):
    ...
    if REPOSITORIES is not None and not DEMO_MODE: inv=REPOSITORIES['invoices'].get(req.invoice_id)
    if not inv: raise HTTPException(404,'Invoice not found')
...
def create_purchase(req:PurchaseRequest,user=Depends(require_permission('create'))):
    company_scope(user,req.company_id); pid=str(uuid.uuid4()); row={'id':pid,**req.model_dump()}; PURCHASES[pid]=row; audit('CREATE','PURCHASE',pid,row); persist_state(); return row

@app.get('/api/purchases')
def purchases(company_id:str='demo-company',user=Depends(require_permission('read'))):
    company_scope(user,company_id)
    if REPOSITORIES is not None and not DEMO_MODE: return REPOSITORIES['purchases'].list_by_company(company_id)
...
def reconciliation(period:str='2026-09',company_id:str='demo-company',user=Depends(require_permission('read'))):
    company_scope(user,company_id)
    if REPOSITORIES is not None and not DEMO_MODE:
        # Production reconciliation is repository-backed.
        purchases=[dict(x) for x in REPOSITORIES['purchases'].list_by_company(company_id,period)]
        entries=[dict(x) for x in REPOSITORIES['reconciliation'].list_2b(company_id,period)]
        result=[]
        for p in purchases:
            ...
            if not matches: result.append({'purchase_id':str(p['id']),'invoice_no':p['invoice_number'],'book_total':str(p['total']),'gstr2b_status':'MISSING','bucket':'Missing in GSTR-2B'})
                ...
                result.append({'purchase_id':str(p['id']),'invoice_no':p['invoice_number'],'book_total':str(p['total']),'gstr2b_status':'MATCHED' if same else 'VALUE_MISMATCH','bucket':'OK' if same else 'Value mismatch'})
    ...
    for p in purchases:
        ...
        if not matches: result.append({'purchase_id':p['id'],'invoice_no':p['invoice_number'],'book_total':p['total'],'gstr2b_status':'MISSING','bucket':'Missing in GSTR-2B'})
            ...
            m=matches[0]; same=abs(float(m['total'])-float(p['total']))<0.01; result.append({'purchase_id':p['id'],'invoice_no':p['invoice_number'],'book_total':p['total'],'gstr2b_status':'MATCHED' if same else 'VALUE_MISMATCH','bucket':'OK' if same else 'Value mismatch'})
    return {'period':period,'rows':result,'summary':{'ok':sum(r['bucket']=='OK' for r in result),'exceptions':sum(r['bucket']!='OK' for r in result)}}

def gstr1_data(period,company_id):
    if REPOSITORIES is not None and not DEMO_MODE:
        return {'period':period,'b2b':[],'b2c':[],'cdnr':[],'hsn_summary':[]}
    docs=[]; hsn={}; b2cs=[]; cdnr=[]
    ...
    def clean(v): return str(v.quantize(Decimal('0.01'))) if isinstance(v,Decimal) else v
...
def gstr1(period:str='2026-09',company_id:str='demo-company',user=Depends(require_permission('read'))):
    company_scope(user,company_id); d=gstr1_data(period,company_id); d.update({'status':'DRAFT','filing_ready':False,'schema_version':'GSTR1-DRAFT-v1','tables':{'4A_B2B':len(d['b2b']),'5_B2CL':0,'7_B2CS':len(d['b2c']),'9B_CDNR':len(d['cdnr']),'12_HSN':len(d['hsn_summary'])},'gstn_payload':{'gstin':next((g['gstin'] for g in GSTINS.values() if _canonical_company_id(g['company_id'])==_canonical_company_id(company_id),''),'fp':period.replace('-',''),'b2b':d['b2b'],'b2cs':d['b2c'],'cdnr':d['cdnr'],'hsn':d['hsn_summary']}}); return d
...
def gstr3b(period:str='2026-09',company_id:str='demo-company',user=Depends(require_permission('read'))):
    company_scope(user,company_id); d=gstr1_data(period,company_id); out={'taxable':Decimal('0'),'cgst':Decimal('0'),'sgst':Decimal('0'),'igst':Decimal('0')}
    ...
    itc={k:sum(Decimal(str(p[k])) for p in purchases) for k in ('cgst','sgst','igst')}; outward={k:str(v.quantize(Decimal('0.01'))) for k,v in out.items()}; available={k:str(v.quantize(Decimal('0.01'))) for k,v in itc.items()}; net={k:str((out[k]-itc[k]).quantize(Decimal('0.01'))) for k in ('cgst','sgst','igst')}
...
def submit_invoice(invoice_id:str,user=Depends(require_permission('edit'))):
    ...
    if not inv: raise HTTPException(404,'Invoice not found')
    company_scope(user,inv['request'].get('company_id','')); ensure_period_open(inv['request'].get('gstin_id',''),inv['request']['invoice_date'])
    if inv.get('status') not in ('DRAFT','REJECTED'): raise HTTPException(409,'Only draft or rejected invoices can be submitted for approval.')
    ...
    try:
        if REPOSITORIES is not None and not DEMO_MODE:
            inv,approval=REPOSITORIES['transactions'].submit_invoice(invoice_id,approval,{'id':str(uuid.uuid4()),'action':'SUBMIT_APPROVAL','entity_type':'INVOICE','entity_id':invoice_id,'new_value':approval,'company_id':inv['request']['company_id'],'user_id':user['sub'],'created_at':datetime.now(timezone.utc).isoformat(),'previous_hash':'GENESIS','hash':''})
        else:
            inv['status']='PENDING_APPROVAL'; APPROVALS[invoice_id]=approval; audit('SUBMIT_APPROVAL','INVOICE',invoice_id,approval,company_id=inv['request']['company_id'],user_id=user['sub'])
    except Exception as exc: raise HTTPException(409,f'Invoice approval submission failed: {exc}')
    persist_state(); return approval
...
def approve_invoice(invoice_id:str,req:ApprovalRequest,user=Depends(require_permission('approve'))):
    ...
    if REPOSITORIES is not None and not DEMO_MODE: approval=REPOSITORIES['approvals'].get(invoice_id)
...
def approvals(company_id:str='demo-company',user=Depends(require_permission('read'))):
    company_scope(user,company_id)
    if REPOSITORIES is not None and not DEMO_MODE: return REPOSITORIES['approvals'].list_by_company(company_id)
...
def lock_return(req:ReturnLockRequest,user=Depends(require_permission('lock'))):
    ...
    if REPOSITORIES is not None and not DEMO_MODE:
        actual=REPOSITORIES['returns'].save({'id':str(uuid.uuid4()),'gstin_id':req.gstin_id,'return_type':req.return_type,'period':req.period,'status':'LOCKED','filed_on':None,'json_file':{}})
        return actual
    RETURNS[key]={'id':str(uuid.uuid4()),**req.model_dump(),'status':'LOCKED','locked_at':datetime.now(timezone.utc).isoformat()}; audit('LOCK_RETURN','RETURN_PERIOD',RETURNS[key]['id'],RETURNS[key],company_id=GSTINS.get(req.gstin_id,{}).get('company_id','demo-company'),user_id=user['sub']); persist_state(); return RETURNS[key]

@app.get('/api/returns')
def returns(gstin_id:str='demo-gstin'):
...
def audit_verify(company_id:str='demo-company',user=Depends(require_permission('read'))):
    company_scope(user,company_id)
    if REPOSITORIES is not None and not DEMO_MODE:
        rows=REPOSITORIES['audit'].list_by_company(company_id)
    else: rows=[x for x in AUDIT if x['company_id']==company_id]
    previous='GENESIS'
    for x in rows:
...
def audit_logs(company_id:str='demo-company',user=Depends(require_permission('read'))):
    company_scope(user,company_id)
    if REPOSITORIES is not None and not DEMO_MODE: return REPOSITORIES['audit'].list_by_company(company_id)
...
def login(req:AuthRequest):
    if REPOSITORIES is not None and not DEMO_MODE: u=REPOSITORIES['masters'].get_user_by_email(req.email)
    else: u=next((x for x in USERS.values() if x['email']==req.email),None)
...
def users(company_id:str='demo-company',user=Depends(require_permission('admin'))):
    company_scope(user,company_id); return [{k:v for k,v in x.items() if k!='password_hash'} for x in USERS.values() if _canonical_company_id(x['company_id'])==_canonical_company_id(company_id)]
...
def create_user(req:UserRequest,user=Depends(require_permission('admin'))):
    ...
    uid=str(uuid.uuid4()); row={'id':uid,'company_id':req.company_id,'name':req.name,'email':req.email,'role':req.role,'password_hash':hash_password(req.password)}; USERS[uid]=row; audit('CREATE','USER',uid,{k:v for k,v in row.items() if k!='password_hash'},company_id=req.company_id,user_id=user['sub']); persist_state(); return {k:v for k,v in row.items() if k!='password_hash'}

@app.get('/api/dashboard')
def dashboard(period:str='2026-09',company_id:str='demo-company',user=Depends(require_permission('read'))):
    company_scope(user,company_id)
    sales=REPOSITORIES['invoices'].list_by_company(company_id,period) if REPOSITORIES is not None and not DEMO_MODE else [x for x in INVOICES.values() if x['request'].get('company_id')==company_id and x['request']['invoice_date'][:7]==period]
    ...
    purchases=REPOSITORIES['purchases'].list_by_company(company_id,period) if REPOSITORIES is not None and not DEMO_MODE else [x for x in PURCHASES.values() if x['company_id']==company_id and x['invoice_date'][:7]==period]
    itc=sum(Decimal(str(x['cgst']))+Decimal(str(x['sgst']))+Decimal(str(x['igst'])) for x in purchases if x.get('itc_eligible',True))
    aato=COMPANIES.get(company_id,{}).get('aato',0) if DEMO_MODE else (REPOSITORIES['companies'].get(company_id) or {}).get('aato',0)
    return {'period':period,'invoice_count':len(sales),'taxable_sales':str(taxable),'output_tax':str(tax),'itc_available':str(itc),'net_tax':str(tax-itc),'aato':aato,'e_invoice_applicable':aato>=50000000,'e_invoice_30day_rule':aato>=100000000,'gstr1_status':'DRAFT','gstr3b_status':'DRAFT'}
