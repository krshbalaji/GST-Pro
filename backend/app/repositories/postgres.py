...
class PostgresCompanyRepository(_Base):
    ...
    def create(self, row: dict, conn=None):
        return self._one(
            ...
            (_uuid(row.get("id") or uuid4()), row["legal_name"], row.get("trade_name"),
             row["pan"], _decimal(row.get("aato")), __import__("json").dumps(row.get("address") or {})),
            conn,
        )
    ...
    def save(self, row: dict, conn=None):
        result = self._one(
            ...
            (row["legal_name"], row.get("trade_name"), row["pan"], _decimal(row.get("aato")),
             __import__("json").dumps(row.get("address") or {}), _uuid(row["id"])),
            conn,
        )
...
class PostgresMasterRepository(_Base):
    ...
    def create(self, kind: str, row: dict, conn=None):
        ...
        if kind == "gstins":
            ...
            values = (
                entity_id, _uuid(row["company_id"]), row.get("name"), row["email"], row["role"], row["password_hash"],
                row.get("is_active", True), row.get("last_login_at")
            )
        return self._one(
            f"INSERT INTO {table} ({cols}) VALUES ({','.join(['%s'] * len(values))}) RETURNING *",
            values, conn
        )
    ...
    def save(self, kind: str, row: dict, conn=None):
        ...
        if kind == "gstins":
            ...
            values = (
                _uuid(row["company_id"]), row["gstin"], row["state_code"], row["scheme"], row.get("legal_name"),
            ...
            values = (
                _uuid(row["company_id"]), row["name"], row.get("gstin"), row.get("state_code"),
            ...
            values = (
                _uuid(row["company_id"]), row["description"], row.get("hsn_sac"), row.get("unit"),
                _decimal(row.get("rate")), _decimal(row.get("gst_rate")), row.get("taxable", True),
                row.get("is_service", False), row.get("active", True), entity_id
            )
        else:
            ...
            values = (
                _uuid(row["company_id"]), row.get("name"), row["email"], row["role"], row["password_hash"],
...
class PostgresInvoiceRepository(_Base):
    def _invoice_company_id(self, invoice_id: UUID, conn=None):
        row = self._one(
            "SELECT g.company_id FROM invoices i JOIN gstins g ON g.id=i.gstin_id WHERE i.id=%s",
            (invoice_id,), conn
        )
        return row["company_id"] if row else None

    def create(self, row: dict, conn=None):
        request, calc = row["request"], row["calculation"]
        invoice_id = _uuid(row.get("id") or uuid4())
        gstin_id = _uuid(request["gstin_id"])
        company_row = self._one("SELECT company_id FROM gstins WHERE id=%s", (gstin_id,), conn)
        if not company_row or str(company_row["company_id"]) != str(request.get("company_id")):
            raise ValueError("GSTIN does not belong to requested company")
        company_id = company_row["company_id"]

        customer_id = None
        customer_ref = request.get("customer_ref") or request.get("customer_id")
        if customer_ref:
            customer_id = _uuid(customer_ref)
            if not self._one(
                "SELECT id FROM customers WHERE id=%s AND company_id=%s",
                (customer_id, company_id), conn
            ):
                raise ValueError("Customer reference not found in company")
        elif request.get("customer_gstin"):
            customer = self._one(
                "SELECT id FROM customers WHERE company_id=%s AND gstin=%s LIMIT 1",
                (company_id, request["customer_gstin"]), conn
            )
            if customer:
        ...
        if request.get("original_invoice_id"):
            ...
            original = self._one(
                "SELECT id,gstin_id,type,status FROM invoices WHERE id=%s",
                (original_invoice_id,), conn
            )
            if not original:
                raise ValueError("Original invoice not found")
            if original["gstin_id"] != gstin_id or self._invoice_company_id(original_invoice_id, conn) != company_id:
                raise ValueError("Original invoice must belong to the same GSTIN/company")
            if request.get("invoice_type") not in ("CREDIT_NOTE","DEBIT_NOTE"):
                raise ValueError("original_invoice_id is only valid for credit/debit notes")
            if request.get("invoice_type") == "CREDIT_NOTE" and original["type"] != "TAX_INVOICE":
                raise ValueError("Credit Note must reference a tax invoice")
            if request.get("invoice_type") == "DEBIT_NOTE" and original["type"] != "TAX_INVOICE":
                raise ValueError("Debit Note must reference a tax invoice")

        if request.get("invoice_type") in ("CREDIT_NOTE","DEBIT_NOTE") and not original_invoice_id:
            raise ValueError("Credit/Debit Note requires original_invoice_id")

        self._one(
            "SELECT id FROM invoices WHERE gstin_id=%s AND series=%s AND number=%s FOR UPDATE",
            (gstin_id, request.get("series"), request["invoice_number"]), conn
        )
        self._one(
            """INSERT INTO invoices
               (id,gstin_id,customer_id,original_invoice_id,type,invoice_date,series,number,place_of_supply,reverse_charge,
                subtotal,cgst,sgst,igst,total,status,version)
               VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) RETURNING id""",
            (
                invoice_id, gstin_id, customer_id, original_invoice_id, request.get("invoice_type","TAX_INVOICE"),
                _date(request["invoice_date"]), request.get("series"), request["invoice_number"], request.get("place_of_supply"),
                request.get("reverse_charge",False), _decimal(calc.get("taxable_value")), _decimal(calc.get("cgst")),
                _decimal(calc.get("sgst")), _decimal(calc.get("igst")), _decimal(calc.get("total")), row.get("status","DRAFT"), 1
            ), conn
        )

        for line in calc.get("lines", []):
            product_id = line.get("product_id")
            if product_id and not self._one(
                "SELECT id FROM products WHERE id=%s AND company_id=%s AND active=true",
                (_uuid(product_id), company_id), conn
            ):
                raise ValueError("Product reference not found in company")
            self._one(
    ...
    def _load(self, invoice_id: UUID, conn=None, for_update=False):
        ...
        original = self._one(
            "SELECT id,type,invoice_date,series,number,status FROM invoices WHERE id=%s",
            (inv["original_invoice_id"],), conn
        ) if inv["original_invoice_id"] else None
        request = {
            "invoice_type": inv["type"], "invoice_date": inv["invoice_date"].isoformat(),
            "series": inv["series"], "invoice_number": inv["number"],
            "original_invoice_id": str(inv["original_invoice_id"]) if inv["original_invoice_id"] else None,
            "place_of_supply": inv["place_of_supply"], "reverse_charge": inv["reverse_charge"],
            "gstin_id": str(inv["gstin_id"]), "company_id": str(gstin["company_id"]) if gstin else "",
            ...
            "scheme": gstin["scheme"] if gstin else "REGULAR", "lines": [],
        }
        calc = {
            "taxable_value": str(inv["subtotal"] or 0), "cgst": str(inv["cgst"] or 0),
            "sgst": str(inv["sgst"] or 0), "igst": str(inv["igst"] or 0), "total": str(inv["total"] or 0),
        ...
        out = {"id":str(inv["id"]), "created_at":inv["created_at"].isoformat() if inv["created_at"] else None,
               "request":request, "calculation":calc, "status":inv["status"]}
        if original:
            out["original_invoice"] = {
                "id":str(original["id"]), "type":original["type"], "invoice_date":original["invoice_date"].isoformat(),
                "series":original["series"], "invoice_number":original["number"], "status":original["status"]
            }
        return out
    ...
    def get_for_update(self, invoice_id: str, conn):
        return self._load(_uuid(invoice_id), conn, True)
    ...
    def save(self,row:dict,conn=None,expected_status:Optional[str]=None,immutable=True)->dict:
        ...
        if immutable and (
            req.get("series")!=current["request"].get("series")
            or req.get("invoice_number")!=current["request"].get("invoice_number")
        ):
            raise ValueError("Invoice series/number is immutable after creation")
        if req.get("original_invoice_id") != current["request"].get("original_invoice_id"):
            raise ValueError("Original invoice reference is immutable after creation")
        ...
        self._one(
            """UPDATE invoices SET type=%s,invoice_date=%s,place_of_supply=%s,reverse_charge=%s,subtotal=%s,cgst=%s,sgst=%s,igst=%s,total=%s,status=%s,version=version+1,updated_at=now()
               WHERE id=%s""",
            (
                req.get("invoice_type","TAX_INVOICE"), _date(req["invoice_date"]), req.get("place_of_supply"),
                req.get("reverse_charge",False), _decimal(calc.get("taxable_value")), _decimal(calc.get("cgst")),
                _decimal(calc.get("sgst")), _decimal(calc.get("igst")), _decimal(calc.get("total")), target, invoice_id
            ), conn
        )
        return self._load(invoice_id,conn)

    def list_all(self):
        return [self.get(str(row["id"])) for row in self._all("SELECT id FROM invoices ORDER BY id")]
    ...
    def save(self,row:dict,conn=None):
        return self._one(
            """INSERT INTO return_periods (id,gstin_id,return_type,period,status,filed_on,json_file)
               ...
               RETURNING *""",
            (_uuid(row.get("id") or uuid4()),_uuid(row["gstin_id"]),row["return_type"],row["period"],row.get("status","DRAFT"),
             row.get("filed_on"),__import__("json").dumps(row.get("json_file") or {})),conn)


class PostgresApprovalRepository(_Base):
    def get(self,invoice_id:str):
        row=self._one("SELECT * FROM invoice_approvals WHERE invoice_id=%s",(_uuid(invoice_id),))
        return dict(row) if row else None
    def get_for_update(self,invoice_id:str,conn):
        row=self._one("SELECT * FROM invoice_approvals WHERE invoice_id=%s FOR UPDATE",(_uuid(invoice_id),),conn)
        return dict(row) if row else None
    def save(self,approval:dict,conn=None):
        ...
        return self._one(
            ...
            (uuid4(),invoice_id,approval["status"],_uuid(approval["submitted_by"]) if approval.get("submitted_by") else None,
             ...
             approval.get("approved_at"),approval.get("comment")),conn)
...
class PostgresPurchaseRepository(_Base):
    def list_by_company(self,company_id:str,period:Optional[str]=None):
        ...
        if period:
            sql+=" AND to_char(e.invoice_date,'YYYY-MM')=%s"; params.append(period)
        return self._all(sql,tuple(params))
    def create(self,row:dict,conn=None):
        return self._one(
            """INSERT INTO gstr2b_entries (id,gstin_id,vendor_gstin,vendor_name,invoice_number,invoice_date,taxable_value,cgst,sgst,igst,total,source)
               VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) RETURNING *""",
            (_uuid(row.get("id") or uuid4()),_uuid(row["gstin_id"]),row["vendor_gstin"],row.get("vendor_name"),
             row["invoice_number"],_date(row["invoice_date"]),_decimal(row.get("taxable_value")),_decimal(row.get("cgst")),
             _decimal(row.get("sgst")),_decimal(row.get("igst")),_decimal(row.get("total")),row.get("source","MANUAL")),conn)

    def save(self,row,conn=None):
        return self._one(
            """UPDATE gstr2b_entries SET vendor_name=%s,invoice_number=%s,invoice_date=%s,taxable_value=%s,cgst=%s,sgst=%s,igst=%s,total=%s,source=%s WHERE id=%s RETURNING *""",
            (row.get("vendor_name"),row["invoice_number"],_date(row["invoice_date"]),_decimal(row.get("taxable_value")),
             _decimal(row.get("cgst")),_decimal(row.get("sgst")),_decimal(row.get("igst")),_decimal(row.get("total")),
             row.get("source","MANUAL"),_uuid(row["id"])),conn)
...
class PostgresReconciliationRepository(_Base):
    ...
    def upsert_2b(self,row:dict,conn=None):
        ...
        return PostgresPurchaseRepository(self.store).save(row,conn)
...
class PostgresEInvoiceRepository(_Base):
    def get(self,invoice_id:str):
        row=self._one("SELECT * FROM e_invoices WHERE invoice_id=%s",(_uuid(invoice_id),))
        return dict(row) if row else None
    def save(self,row:dict,conn=None):
        return self._one(
